...
function processJsonData(jsonData) {
    ...
    const datasets = topTags.map((tag, index) => {
        ...
        return {
            label: tag,
            data: labels.map(year => (yearData[year] * 100).toFixed(1)), // Convert to percentage string
            absoluteData: labels.map(year => yearData[year]), // Original decimal values
...
fetch('https://dhp-project.onrender.com/data') // assuming JSON is returned now
